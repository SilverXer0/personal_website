...
import { ArrowRight, Download, Github, Linkedin, Mail, Moon, Sun, Globe, ExternalLink, MapPin, GraduationCap, Cpu, Code2, Rocket, ChevronRight, Trophy } from 'lucide-react'
...
export default function Page() {
  ...
  const skills = useMemo(
    () => [
      { group: 'Languages', items: ['TypeScript', 'JavaScript', 'Java', 'Swift', 'Python', 'C', 'C++', 'C#', 'Kotlin', 'Racket'] },
      { group: 'Frameworks & Tools', items: ['React/Next.js', 'Tailwind CSS', 'Node.js', 'Express', 'Android Studio', 'Xcode', 'Unity', 'Unreal', 'Git/GitHub'] },
      { group: 'Data & Systems', items: ['PostgreSQL', 'MySQL', 'Redis', 'Docker', 'GCP', 'AWS'] },
    ],
    []
  )
  ...
  const projects = useMemo(
      ...
      },
    {
        ...
        tech: ['Unity', 'C#', 'Python'],
        links: [ { label: 'Repo', href: 'https://github.com/SilverXer0' } ], 
      },
           {
        name: 'MintMatch',
      ...
      <div className="min-h-screen bg-neutral-50 text-neutral-900 dark:bg-neutral-950 dark:text-neutral-100 transition-colors duration-300">
        ...
        <nav className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/40 dark:supports-[backdrop-filter]:bg-black/30 bg-white/60 dark:bg-black/40 border-b border-black/5 dark:border-white/10">
          <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button aria-label="Toggle theme" onClick={() => setDark(d => !d)} className="rounded-xl border border-black/10 dark:border-white/10 p-2 hover:shadow-sm">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="lg:col-span-3">
              ...
              <p className="mt-4 text-base sm:text-lg text-neutral-600 dark:text-neutral-300 max-w-2xl">
                I'm a rising professional working with different tech stacks, such as full-stack development, mobile app development, and machine learning. Here, you’ll find class projects, research, and real‑world work with companies like Apple and global organizations like Hack4Impact.
              </p>
              <div className="mt-6 flex flex-wrap items-center gap-3">
                <a href="#contact" className="inline-flex items-center gap-2 rounded-2xl bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 px-4 py-2 font-medium shadow hover:shadow-md">
                  Email Me <Mail className="h-4 w-4" />
                </a>
                <a href="https://github.com/SilverXer0" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-2xl border border-black/10 dark:border-white/10 px-4 py-2 font-medium hover:shadow-sm">
                  Github <Github className="h-4 w-4" />
                </a>
                <a href="https://www.linkedin.com/in/sharankrishna14/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-2xl border border-black/10 dark:border-white/10 px-4 py-2 font-medium hover:shadow-sm">
              ...
              <div className="relative overflow-hidden rounded-3xl border border-black/10 dark:border-white/10 p-6 bg-gradient-to-b from-neutral-100 to-white dark:from-neutral-900 dark:to-neutral-950">
                <div className="flex items-start justify-between">
                  <div className="text-sm text-neutral-600 dark:text-neutral-300">
                    ...
                    <div className="flex items-center gap-1 mt-1"><GraduationCap className="h-4 w-4" /> Cal Poly, SLO</div>
                    <div className="flex items-center gap-1 mt-1"><MapPin className="h-4 w-4" /> San Luis Obispo, CA</div>
                  </div>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  {[
                    { label: 'Expected', value: 'December 2025' }
                  ].map((card) => (
                    <div key={card.label} className="rounded-2xl border border-black/10 dark:border-white/10 p-3 text-sm">
        ...
        <Section id="about" titleIcon={<Code2 className="h-5 w-5" />} title="About">
          <p className="mt-3 text-neutral-700 dark:text-neutral-300 leading-relaxed">
            I build software with an eye for product polish and scalability and reliability. Recently I’ve been exploring <strong>AI‑assisted tooling</strong>, <strong>procedural generation for games</strong>, and <strong>metrics‑driven experimentation</strong>. In class, I’ve implemented parsers and interpreters (Racket), schedulers, and memory models; outside class, I’ve shipped web apps and mobile prototypes used by real people and companies.
          </p>
          <ul className="mt-4 grid sm:grid-cols-2 gap-3 text-sm text-neutral-600 dark:text-neutral-300">
            <li className="flex items-start gap-2"><ChevronRight className="h-4 w-4 mt-0.5" /> Seeking: Connections, Collaborations, and any other Opportunites. </li>
            <li className="flex items-start gap-2"><ChevronRight className="h-4 w-4 mt-0.5" /> Interests: Machine Learning, Swift Development, Cloud, and Game Tech.</li>
            <li className="flex items-start gap-2"><ChevronRight className="h-4 w-4 mt-0.5" /> Currently: Polishing Unity ML Aim Trainer.</li>
            <li className="flex items-start gap-2"><ChevronRight className="h-4 w-4 mt-0.5" /> Outside Fun: Valorant, Basketball, PC parts and peripherals.</li>
          </ul>
        </Section>
              ...
              <Card key={job.title}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="text-neutral-500 text-sm">{job.org} · {job.period}</div>
                    <div className="text-neutral-500 text-xs flex items-center gap-1 mt-1"><MapPin className="h-3.5 w-3.5" /> {job.location}</div>
                  </div>
                </div>
                <ul className="mt-3 space-y-2 text-sm text-neutral-700 dark:text-neutral-300">
                  {job.bullets.map((b, i) => (
                    <li key={i} className="flex gap-2"><ChevronRight className="h-4 w-4 flex-none mt-0.5" /> <span>{b}</span></li>
                  ))}
                </ul>
              </Card>
            ))}
          ...
          <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((p) => (
              <Card key={p.name}>
                ...
                <p className="mt-3 text-sm text-neutral-700 dark:text-neutral-300">{p.desc}</p>
                <div className="mt-3 flex flex-wrap gap-2 text-xs text-neutral-600 dark:text-neutral-300">
                  {p.tech.map((t) => (
                    <span key={t} className="rounded-full border border-black/10 dark:border-white/10 px-2 py-1">{t}</span>
                  ))}
                </div>
                {p.links?.length ? (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {p.links.map((l) => (
                      <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-xl border border-black/10 dark:border-white/10 px-3 py-1 text-xs hover:shadow-sm">
                ...
                {skills.map((s) => (
                  <div key={s.group}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {s.items.map((i) => (
                        <span key={i} className="rounded-full border border-black/10 dark:border-white/10 px-2 py-1 text-sm">{i}</span>
                      ))}
                    </div>
                  </div>
                ))}
        ...
        <Section id="writing" titleIcon={<PenIcon />} title="Writing">
          <div className="mt-6 grid md:grid-cols-2 gap-6">
            {[
              { title: 'The Evolution of Algorithms and Techniques of Load Balancing in Distributed Systems', desc: 'Survey Paper on the history of load balancing in Distributed Systems.', href: '/papers/load-balancing.pdf' },
            ].map((w) => (
              <a key={w.title} href={w.href} target="_blank" rel="noopener noreferrer" className="rounded-2xl border border-black/10 dark:border-white/10 p-5 bg-white/60 dark:bg-white/5 hover:shadow-sm transition">
                <div className="font-semibold">{w.title}</div>
                <p className="text-sm text-neutral-600 dark:text-neutral-300 mt-1">{w.desc}</p>
          ...
          <div className="mt-6">
            <Card>
              <p className="text-neutral-700 dark:text-neutral-300">
                I’m always down to chat about job opportunites, research, or fun side projects. Feel free to email me or ping me on LinkedIn!.
              </p>
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <a href="mailto:krishna.sharan@gmail.com" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-2xl bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 px-4 py-2 font-medium shadow hover:shadow-md">
...
function Section({ id, title, titleIcon, children }: { id?: string; title: string; titleIcon?: React.ReactNode; children: React.ReactNode }) {
  return (
...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="rounded-2xl border border-black/10 dark:border-white/10 p-5 bg-white/60 dark:bg-white/5">
      {children}
    </motion.div>
  )
}
...
function PenIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-5 w-5">
      <path d="M16.862 3.487a2.5 2.5 0 0 1 3.535 3.535l-11.2 11.2a4 4 0 0 1-1.702.986l-3.45.985.986-3.45a4 4 0 0 1 .985-1.701l11.2-11.2Z" className="stroke-current" strokeWidth="1.5"/>
      <path d="M14.75 5.6l3.536 3.536" className="stroke-current" strokeWidth="1.5"/>
    </svg>
  )
}